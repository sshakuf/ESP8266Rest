...
DateTime GetDateTime(unsigned long epoch)
{
	...
	retVal.year=get_year(&epoch);
	retVal.month=get_month(&epoch,retVal.year);
	retVal.day=1+(epoch/86400);
	epoch=epoch%86400;
	retVal.hour=epoch/3600;
	epoch%=3600;
	retVal.min=epoch/60;
	retVal.sec=epoch%60;
...
DateTime Now()
{
	return GetDateTime(sntp_time+(sntp_tz*3600));
}
